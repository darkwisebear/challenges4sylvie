...
class PerPiecePosition{
public:
    PerPiecePosition(char Artikelp, float Betragp, int Anzahlp)
	{
		this-> Artikelp = Artikelp;
		this-> Betragp = Betragp;
		this-> Anzahlp = Anzahlp;
	}
private:
	char Artikelp;
	float Betragp;
...
class WeightPosition{
public:
    WeightPosition(char Artikelw, float Betragw, float Gewichtw)
	{
		this-> Artikelw = Artikelw;
		this-> Betragw = Betragw;
		this-> Gewichtw = Gewichtw;
	}
	private:
	char Artikelw;
	float Betragw;
...
{
	public:
	void add_position(float sum);
	void get_position(float sum);
	void num_positions(float sum);
	void total_price(float sum);
};

void Cashbox::add_position(float sum)
{
};

void Cashbox::get_position(float sum)
{
};

void Cashbox::num_positions(float sum)
{
};

void Cashbox::total_price(float sum)
{
};
